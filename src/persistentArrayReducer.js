...
const initializePersistentArrayReducer = async (storeGetter,
                                                db,
                                                reducerName,
                                                saveArrayReducer) => {
  try {
    ...
    db.changes({
      ...
      if (change.doc) {
        ...
        change.doc._deleted ? removeArrayReducer(store, change.doc, reducerName) : updateArrayReducer(store, change.doc, reducerName)
      } else {
...
const persistentArrayReducer = (storeGetter, db, reducerName) => reducer => {
  ...
  return (state, action) => {
    ...
    else if (action.type === REMOVE_ARRAY_REDUCER &&
      action.reducerName === reducerName &&
      action.doc) {
      lastState = state.filter(item => {
        return !equalsOmittingDocProps(item, action.doc) && item._id !== action.doc._id
      });
      return reducer(lastState, action);
    }
